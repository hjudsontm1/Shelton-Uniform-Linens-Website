...
// Preserve the approved vehicle lockup as one unit, then match its entire
// rotation to the route stripe for this requested concept pass.
const fullLockup = await renderSvg(vehicleLightSvg, 870);
...
// These are the unchanged v5 route-stripe and gold-cap polygons requested by
// the user.
...
const output = path.join(
  outputDir,
  "shelton-mt45-side-option-2-route-stripe-full-lockup-rotation-v7.png",
);
...
await sharp(sidePhoto)
  .composite([
    ...
    { input: rotatedFullLockup, left: 780, top: 455 },
