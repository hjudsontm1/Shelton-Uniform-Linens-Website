...
const { pricingRules, buildEstimateInput, calculatePlanningRange } = global.window.SheltonPricingDevelopmentRules;
...
(async () => {
  ...
  assert.equal(result.rulesVersion, "commercial-estimator.v2");
  ...
  assert.equal(calls[0].url, "/api/public/commercial-estimate");
  ...
  console.log("Commercial Estimator V2 website adapter tests passed.");
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
