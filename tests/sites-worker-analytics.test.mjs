...
const analyticsBatch = (overrides = {}, now = Date.now()) => ({
  ...
  events: [
    {
      ...
      pageKey: "pricing",
      pagePath: "/private/jordan@example.test?secret=value#factor-route",
      viewport: "compact",
...
test("website analytics forwards only the Office contract through the trusted proxy", async () => {
  ...
  try {
    ...
    assert.doesNotMatch(JSON.stringify(forwarded), /203\.0\.113\.42|private@example|jordan@example|secret=value/);
...
test("analytics fingerprint is stable within a UTC day and rotates the next day", async () => {
  const originalFetch = globalThis.fetch;
  ...
  globalThis.fetch = async (_url, options) => {
    fingerprints.push(new Headers(options.headers).get("x-shelton-client-fingerprint"));
    return acceptedResponse();
  };

  try {
    await worker.fetch(analyticsRequest(analyticsBatch({}, now)), env);
    now = base + 60 * 60_000;
    await worker.fetch(analyticsRequest(analyticsBatch({}, now)), env);
    now = base + 24 * 60 * 60_000;
    await worker.fetch(analyticsRequest(analyticsBatch({}, now)), env);
...
test("analytics fingerprint is purpose-separated from commercial API traffic", async () => {
  const originalFetch = globalThis.fetch;
  const originalDateNow = Date.now;
  ...
  Date.now = () => base;
  globalThis.fetch = async (url, options) => {
    fingerprints.set(new URL(url).pathname, new Headers(options.headers).get("x-shelton-client-fingerprint"));
    if (url.endsWith("website-events")) return acceptedResponse();
    return new Response(JSON.stringify({ error: "Rejected for test", code: "test_rejection" }), {
      status: 400,
  ...
  try {
    const analytics = await worker.fetch(analyticsRequest(analyticsBatch({}, base)), env);
    assert.equal(analytics.status, 202);
    const estimate = await worker.fetch(new Request(`${SITE_ORIGIN}/api/commercial-estimate`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "CF-Connecting-IP": "203.0.113.42" },
      body: "{}"
    }), env);
    assert.equal(estimate.status, 400);
    assert.match(fingerprints.get("/api/public/website-events"), /^[a-f0-9]{64}$/);
    assert.match(fingerprints.get("/api/public/commercial-estimate"), /^[a-f0-9]{64}$/);
    assert.notEqual(
      fingerprints.get("/api/public/website-events"),
      fingerprints.get("/api/public/commercial-estimate")
    );
  } finally {
    Date.now = originalDateNow;
    globalThis.fetch = originalFetch;
...
test("website analytics preserves the approved wholesale and other operation categories", async () => {
  const originalFetch = globalThis.fetch;
  const seen = [];
  globalThis.fetch = async (_url, options) => {
    seen.push(JSON.parse(options.body).events[0].properties.operation);
    return acceptedResponse();
  };
  ...
  try {
    for (const operation of ["wholesale", "other"]) {
      const event = {
        ...analyticsBatch().events[0],
        name: "estimator_operation_selected",
        properties: { operation, stage: "operation" }
      };
      const response = await worker.fetch(
        analyticsRequest(analyticsBatch({ events: [event] })),
        env
      );
      assert.equal(response.status, 202);
    }
    assert.deepEqual(seen, ["wholesale", "other"]);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
