(function () {
  ...
  const render = () => {
    root.dataset.activeChapter = state.activeChapter;
    renderConcept();
    renderFlow();
    saveState();
  };
  ...
  const focusChapter = (chapter) => {
    ...
    const positionChapter = () => {
      ...
      window.requestAnimationFrame(() => {
        ...
        const calmSurface = root.classList.contains("pricing-calm") && target.matches("[data-result], [data-quote-handoff]");
        const targetTop = target.getBoundingClientRect().top + window.scrollY - (calmSurface ? 0 : headerHeight + threadHeight + 20);
